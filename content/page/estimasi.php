...
if (isset($_GET['page']) AND $_GET['page'] == "estimasi"){
    ...
    $query1 = mysql_query("SELECT * FROM unit, estimasi WHERE unit.u_kode = '$u_kode' AND estimasi.u_kode = '$u_kode' AND
             estimasi.es_kode = '$es_kode' ")or die (mysql_errno());
    
    //Ambil data Estimasi Jasa
    $query2 = mysql_query("SELECT * FROM estimasijasa WHERE es_kode = '$es_kode' ");
    $querypart = mysql_query("SELECT * FROM estimasipart WHERE es_kode = '$es_kode' ");
